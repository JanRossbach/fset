...
(declare get-statespace create-boolname)
...
(defn setup-backend ;; Call this before testing the backend isolated without an actual translation
  [ir config]
  (let [new-ir (replace-def-sets-with-enum-sets (:deff-set-size config) ir)]
    (reset! db (assoc
                {}
                :ir new-ir
...
(defn finite-type?
  ...
  (if (carrier? expr)
    ...
    (match (get-type-ir expr)
      ...
      {:tag :relation :sets ([A B] :seq)} (and (carrier? A) (carrier? B) (< (count (get-relation-elems A B)) 200))
      _ false)))

(defn finite?
  [expr]
  (match expr
    {:tag :interval :from n :to m} (< (- m n) (:max-unroll-size (:cfg @db)))
    {:tag :cardinality} true
    _ (finite-type? expr)))

(defn unrollable?
  [id]
  (and (not (contains? (:excluded-vars (:cfg @db)) id))
       (finite? id)))
